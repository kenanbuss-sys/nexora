...
/**
 * Sprint 086 acceptance tests (docs/implementation/SPRINT_011_MES.md):
...
integration('Sprint 086 — production labels', () => {
  ...
  const tokenA = identity.signToken({ tenantSlug: 'test-s86a', subject: 'idp|s86-admin' });
  const tokenB = identity.signToken({ tenantSlug: 'test-s86b', subject: 'idp|s86b-admin' });
  ...
  beforeAll(async () => {
    ...
    await api('POST', '/api/v1/tenants', platformToken, {
      slug: 'test-s86a',
      name: 'Sprint86 Tenant A',
      initialAdmin: {
        email: 'admin@s86a.example',
        displayName: 'S86 Admin',
        idpSubject: 'idp|s86-admin',
      },
    });
    await api('POST', '/api/v1/tenants', platformToken, {
      slug: 'test-s86b',
      name: 'Sprint86 Tenant B',
      initialAdmin: {
        email: 'admin@s86b.example',
        displayName: 'S86B Admin',
        idpSubject: 'idp|s86b-admin',
      },
    });
    ...
    await api('POST', '/api/v1/stock/movements', tokenA, {
      ...
      quantity: 20,
      idempotencyKey: 'receipt-s86-bulbs',
    });
  }, 120_000);
  ...
  it('MES-023: completed output aggregates into the by-day report', async () => {
    const report = await api('GET', '/api/v1/work-orders/production-by-day?days=7', tokenA);
    ...
    expect(todayRow!.good).toBeGreaterThan(0);
    expect(todayRow!.workOrders).toBeGreaterThanOrEqual(1);
  });

  it('AUTHZ: the production report needs production.read', async () => {
    const stranger = identity.signToken({ tenantSlug: 'test-s86a', subject: 'idp|s86-nobody' });
    await api('POST', '/api/v1/users/invite', tokenA, {
      email: 'niko81@primjer.example',
      displayName: 'Niko81',
      idpSubject: 'idp|s86-nobody',
    });
    const denied = await api('GET', '/api/v1/work-orders/production-by-day', stranger);
    expect(denied.status).toBe(403);
  });

  it('MES-012: completed scrap spawns exactly one rework order', async () => {
    const list = await api('GET', '/api/v1/work-orders', tokenA);
    const completed = (
      list.body.workOrders as Array<{ id: string; status: string; scrapQuantity: string }>
    ).find((w) => w.status === 'COMPLETED' && Number(w.scrapQuantity) > 0);
    expect(completed).toBeDefined();

    const rework = await api('POST', `/api/v1/work-orders/${completed?.id}/rework`, tokenA);
    expect(rework.status).toBe(201);
    expect(rework.body.status).toBe('PLANNED');
    expect(Number(rework.body.quantity)).toBe(Number(completed?.scrapQuantity));

    const again = await api('POST', `/api/v1/work-orders/${completed?.id}/rework`, tokenA);
    expect(again.status).toBe(409);

    const audit = await prisma.auditEvent.findFirst({ where: { action: 'mes.rework.create' } });
    expect(audit).not.toBeNull();
  });

  it('AUTHZ: rework needs production.manage', async () => {
    const stranger = identity.signToken({ tenantSlug: 'test-s86a', subject: 'idp|s86-nobody' });
    await api('POST', '/api/v1/users/invite', tokenA, {
      email: 'niko86@primjer.example',
      displayName: 'Niko86',
      idpSubject: 'idp|s86-nobody',
    });
    const list = await api('GET', '/api/v1/work-orders', tokenA);
    const anyWo = (list.body.workOrders as Array<{ id: string }>)[0];
    const denied = await api('POST', `/api/v1/work-orders/${anyWo?.id}/rework`, stranger);
    expect(denied.status).toBe(403);
  });

  it('MES-017: a work order renders a printable A6 label PDF', async () => {
    const list = await api('GET', '/api/v1/work-orders', tokenA);
    const anyWo = (list.body.workOrders as Array<{ id: string }>)[0];
    const label = await api('GET', `/api/v1/documents/work-order/${anyWo?.id}/label`, tokenA);
    expect(label.status).toBe(200);
    expect(label.body.contentType).toBe('application/pdf');
    expect(String(label.body.fileName)).toMatch(/-label\.pdf$/);
    const pdf = Buffer.from(label.body.dataBase64 as string, 'base64');
    expect(pdf.subarray(0, 4).toString()).toBe('%PDF');
  });

  it('AUTHZ: labels need production.read', async () => {
    const stranger2 = identity.signToken({ tenantSlug: 'test-s86a', subject: 'idp|s86-nobody2' });
    await api('POST', '/api/v1/users/invite', tokenA, {
      email: 'niko86b@primjer.example',
      displayName: 'Niko86b',
      idpSubject: 'idp|s86-nobody2',
    });
    const list = await api('GET', '/api/v1/work-orders', tokenA);
    const anyWo = (list.body.workOrders as Array<{ id: string }>)[0];
    const denied = await api('GET', `/api/v1/documents/work-order/${anyWo?.id}/label`, stranger2);
    expect(denied.status).toBe(403);
  });
});
