...
/**
 * Sprint 085 acceptance tests (docs/implementation/SPRINT_011_MES.md):
...
integration('Sprint 085 — rework', () => {
  ...
  const tokenA = identity.signToken({ tenantSlug: 'test-s85a', subject: 'idp|s85-admin' });
  const tokenB = identity.signToken({ tenantSlug: 'test-s85b', subject: 'idp|s85b-admin' });
  ...
  beforeAll(async () => {
    ...
    await api('POST', '/api/v1/tenants', platformToken, {
      slug: 'test-s85a',
      name: 'Sprint85 Tenant A',
      initialAdmin: {
        email: 'admin@s85a.example',
        displayName: 'S85 Admin',
        idpSubject: 'idp|s85-admin',
      },
    });
    await api('POST', '/api/v1/tenants', platformToken, {
      slug: 'test-s85b',
      name: 'Sprint85 Tenant B',
      initialAdmin: {
        email: 'admin@s85b.example',
        displayName: 'S85B Admin',
        idpSubject: 'idp|s85b-admin',
      },
    });
    ...
    await api('POST', '/api/v1/stock/movements', tokenA, {
      ...
      quantity: 20,
      idempotencyKey: 'receipt-s85-bulbs',
    });
  }, 120_000);
  ...
  it('AUTHZ: the production report needs production.read', async () => {
    const stranger = identity.signToken({ tenantSlug: 'test-s85a', subject: 'idp|s85-nobody' });
    await api('POST', '/api/v1/users/invite', tokenA, {
      email: 'niko81@primjer.example',
      displayName: 'Niko81',
      idpSubject: 'idp|s85-nobody',
    });
