...
/**
 * Sprint 081 acceptance tests (docs/implementation/SPRINT_011_MES.md):
...
integration('Sprint 081 — shift production report', () => {
  ...
  const tokenA = identity.signToken({ tenantSlug: 'test-s81a', subject: 'idp|s81-admin' });
  const tokenB = identity.signToken({ tenantSlug: 'test-s81b', subject: 'idp|s81b-admin' });
  ...
  beforeAll(async () => {
    ...
    await api('POST', '/api/v1/tenants', platformToken, {
      slug: 'test-s81a',
      name: 'Sprint81 Tenant A',
      initialAdmin: {
        email: 'admin@s81a.example',
        displayName: 'S81 Admin',
        idpSubject: 'idp|s81-admin',
      },
    });
    await api('POST', '/api/v1/tenants', platformToken, {
      slug: 'test-s81b',
      name: 'Sprint81 Tenant B',
      initialAdmin: {
        email: 'admin@s81b.example',
        displayName: 'S81B Admin',
        idpSubject: 'idp|s81b-admin',
      },
    });
    ...
    await api('POST', '/api/v1/stock/movements', tokenA, {
      ...
      quantity: 20,
      idempotencyKey: 'receipt-s81-bulbs',
    });
  }, 120_000);
  ...
  it('AUTHZ: the production report needs production.read', async () => {
    const stranger = identity.signToken({ tenantSlug: 'test-s81a', subject: 'idp|s81-nobody' });
    await api('POST', '/api/v1/users/invite', tokenA, {
      email: 'niko81@primjer.example',
      displayName: 'Niko81',
      idpSubject: 'idp|s81-nobody',
    });
